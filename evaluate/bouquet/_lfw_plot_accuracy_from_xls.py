...
def compile_chart(data, width, title, x, offset, tick_labels, ncol, special=None, upper_label=False, patterns=fantasy,
                  colors=colors, same_color=None):
    fig, ax = plt.subplots()
    ax.set_title(title)
    ...
    ax.set_yticks(np.arange(0.8, 1.01, step=0.1))
    ax.set_yticks(np.arange(0.8, 1.001, step=0.01), minor=True)
...
def plot_bar_chart(corrupted_exp, uncorrupted_exp, filepath, title, by_category=False, debug=False):
    ...
    if by_category:
        create_chart_category(corruptions, data_means_compress, filepath, title)
    else:
    ...
    print(tab_2)
    with open(os.path.splitext(filepath)[0] + "_tab2.txt", 'w') as f:
        f.write(tab_2)
