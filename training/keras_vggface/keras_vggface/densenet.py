...
from antialiasing import LPFConv2D, LPFMaxPooling2D, LPFAveragePooling2D
...
DENSENET121_VGGFACE_WEIGHT_PATH = 'https://www.vvigilante.com/data/densenet121bc_vggface_full.hdf5'
...
def transition_block(x, reduction, name, lpf_size):
    """A transition block.
    ...
    x = layers.Conv2D(int(backend.int_shape(x)[bn_axis] * reduction), 1,
                      use_bias=False,
                      name=name + '_conv')(x)
    x = LPFAveragePooling2D(2, strides=2, name=name + '_pool', lpf_size=lpf_size)(x)
    return x
...
def DenseNet(blocks,
    ...
    x = LPFConv2D(64, 7, strides=2, use_bias=False, name='conv1/conv', lpf_size=lpf_size)(x)
    ...
    if include_top:
        ...
        x = layers.Dense(classes, activation='softmax', name='fc%d'%classes)(x)
    else:
        if pooling == 'avg':
            x = layers.GlobalAveragePooling2D(name='avg_pool')(x)
        elif pooling == 'max':
