...
from keras_vggface.models import SENET, RESNET50, VGG16, SENET50
...
def VGGFace(include_top=True, model='vgg16', weights='vggface',
    ...
    """
    if weights == 'vggface2': weights='vggface'
    ...
    if model == 'vgg16':
        ...
        return VGG16(include_top=include_top, input_tensor=input_tensor,
                     ...
                     weights=weights,
                     classes=classes, lpf_size=lpf_size)
    ...
    if model.startswith('senet'):
        resnet_type=int(model[5:])
        if classes is None:
            classes = 8631
        ...
        return SENET(resnet_type=resnet_type, include_top=include_top, input_tensor=input_tensor,
                        input_shape=input_shape, pooling=pooling,
                        weights=weights, lpf_size=lpf_size,
                        classes=classes)
